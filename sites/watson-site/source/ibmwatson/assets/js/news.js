(function($) { // reset V17e's noConflict
  ...
  IBM.watson.News = function() { this.init(); };
  ...
  IBM.watson.News.prototype = {
    ...
    scene5: function(scene) {
      ...
      scene.timeline
        .from(header, 25, {left: '-100%', opacity: 0}, 'start')
        .from(sprite, 50, {opacity: 0}, 'start')
      ;

      if (true) { // TODO: Only do this if above 640!
        scene.timeline
          .from(callout1, 50, {top: 1000, opacity: 0}, 'start')
          .from(callout2, 50, {top: 1000, opacity: 0}, 'start+=30')
          .from(callout3, 50, {top: 1000, opacity: 0}, 'start+=15')
        ;
      }

      var spriteManager = new IBM.watson.tweens.Sprite(scene);
      spriteManager.buildSpriteTimeline(sprite, {width: 16900, height: 495}, 50, 2);
