...
< 20) $(window).data("ajaxReq", false);
        ...
        $(".ibm_cci__ml-li").on("click", function(event) {
            ...
            if (!$(parentP).hasClass("ibm_cci-clicked")) {
                ...
                if (!$(event.target).hasClass("ibm_cci-close-black")) {
                    ...
                    vo.similar = [];
                    $(".ibm_cci--sr >
    p").addClass("ibm_cci-toggleDisplay");
    ...
    $(window).data("ajaxReq", true).on("scroll", function(event) {
        ...
        if ($(window).data("ajaxReq") == false || vo.totalSearchCountNum
    < 20 || lastRankID < 20) {
            return;
        }
        try {
        ...
        = (($(document).height() + 72) - 900) && $(window).data("ajaxReq", true)) {
                // Set Flag to false
        ...
        <= 500 || $(window).width() <= 800) {
            ...
            vo.f_removeNodes($("#ibm_cci-widget-js >
            span"));
        }
        if ($(window).width()
            <= 500 && $("#ibm_cci-widget-js").hasClass("ibm_cci-lv__modifier")) {
        ...
        if (window.location.hash === "#voices-list" && window.innerWidth > 1280) {
            ...
            $("#ibm_cci-widget-js.ibm-columns").css("width", "auto");
        } else if (window.location.hash === "#voices-grid" && window.innerWidth
                < 1280) {
            $("#ibm_cci-widget-js.ibm-columns").css("width", "");
...
(function($, a) {
    var vo = {
        ...
        f_mod_content: function(data) {
            var update_content = data.replace(/(http(s)*\:\/\/[^\s]+\s*)/g, '
                        <a href="$1">$1</a>
                        ').replace(/#([^\s]+)/g, '
                        <a href="//twitter.com/search?q=%23$1">#$1</a>
                        ').replace(/@([^\s:]+)/g, '
                        <a href="//twitter.com/$1">@$1</a>
                        ');
        ...
        f_pretty_date: function(timeVal) {
            try {
                ...
                if (isNaN(day_diff) || day_diff
                        < 0) return;
                var retVal = day_diff == 0 && (diff < 60 && "just now" || diff < 120 && "1m" || diff < 3600 && Math.floor(diff / 60) + "m" || diff < 7200 && "1h" || diff < 86400 && Math.floor(diff / 3600) + "h") || // day_diff >
                            = 1 && date.getDate() + " " + monthTxt[parseInt(date.getMonth(), 10)];
        ...
        f_addspinner: function() {
            $("#ibm_cci-widget-js").append('
                            <span class="ibm-spinner-small"></span>
                            ');
        },
        ...
        f_removeNodes: function(nodes) {
            // Need to add condition to only remove trending search nodes count >
                                9 
            nodes.remove();
        },
            ...
            try {
                if (searchterms.length === 0) {
                    self.f_fetch_feeds().done(function(data) {
                        self.f_removeNodes($("#ibm_cci-widget-js >
                                    span"));
                        self.search = true;
                        // SEARCH TRUE
...
(function(e, t, n, r) {
    ...
    i.special.smartresize = {
        ...
        handler: function(e, t) {
            var n = this,
                r = arguments;
            e.type = "smartresize", s && clearTimeout(s), s = setTimeout(function() {
    ...
    t.fn.masonry = function(e) {
        if (typeof e == "string") {
            ...
            this.each(function() {
                ...
                if (!r) {
                    o("cannot call methods on masonry prior to initialization; attempted to call method '" + e + "'");
                    return
                }
